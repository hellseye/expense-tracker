...
export function SettingsView() {
  ...
  // Bug Report Modal Form State
  const [issueType, setIssueType] = React.useState("UI / Layout Bug");
  ...
  const handleSendBugReport = (e: React.FormEvent) => {
    ...
    if (!issueDescription.trim()) {
      return toast({
        ...
        description: "Please detail the issue or bug you experienced.",
      });
    }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
            ...
            <Card className="p-6 space-y-6 bg-surface-100/50 border-white/5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-white/5">
                ...
                <select
                  ...
                  className="h-11 rounded-xl border border-border bg-surface-200 px-4 text-sm font-semibold text-foreground focus:border-primary focus:outline-none"
                >
      ...
      <Modal
        ...
        description="Select the type of issue and provide details. This will send an email directly to Mayank with a preset subject line."
      >
        <form onSubmit={handleSendBugReport} className="space-y-4 pt-2">
          <div>
            <label className="block text-xs font-semibold text-zinc-400 mb-1">Type of Issue (Preset Subject)</label>
            <select
              value={issueType}
              onChange={(e) => setIssueType(e.target.value)}
              className="flex h-11 w-full rounded-xl border border-border bg-surface-200 px-3.5 text-sm text-foreground focus:border-primary focus:outline-none"
            >
              <option value="UI / Layout Bug">UI / Layout Bug</option>
              <option value="Authentication & Login Error">Authentication & Login Error</option>
              <option value="Transaction Logging Issue">Transaction Logging Issue</option>
              <option value="Category / Analytics Bug">Category / Analytics Bug</option>
              <option value="Performance & Slow Loading">Performance & Slow Loading</option>
              <option value="Feature Request / Other">Feature Request / Other</option>
            </select>
          </div>

          <div>
            <label className="block text-xs font-semibold text-zinc-400 mb-1">Whole Issue Explanation</label>
            <textarea
              rows={4}
              placeholder="Describe what went wrong, what you expected to happen, or steps to reproduce..."
              value={issueDescription}
              onChange={(e) => setIssueDescription(e.target.value)}
              className="w-full rounded-xl border border-border bg-surface-200 p-3.5 text-sm text-foreground placeholder:text-zinc-500 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30"
            />
          </div>
